...
struct s3c3410x_io_t
{
	...
	uint32_t btcon;
	uint32_t btcnt;
	uint32_t btcnt_scaler;
	struct s3c3410x_timer_t timer[5];
	uint32_t tfcon;
	uint32_t tfstat;
	uint64_t tf4;
	uint32_t tf4_repeat[2];
...
extern uint32_t mem_read_byte(void *, uint32_t);
extern uint32_t mem_read_halfword(void *, uint32_t);
extern uint32_t mem_read_word(void *, uint32_t);
extern void mem_write_byte(void *, uint32_t, uint32_t);
extern void mem_write_halfword(void *, uint32_t, uint32_t);
extern void mem_write_word(void *, uint32_t, uint32_t);

static int s3c3410x_dma_is_valid(int index);
static void s3c3410x_dma_proccess(void  *state, int index);


static void s3c3410x_io_reset(void  *state)
{
	int i;
...
/* Interrupt Routine */
static void s3c3410x_update_int(void  *state)
{
	uint32_t requests = (io.syscon & 0x40) ? (io.intpnd & io.intmsk) : 0x0;
...
static void s3c3410x_interrupt_read(uint32_t addr, uint32_t *data)
{
	switch (addr) {
		case INTMOD:
			*data = io.intmod;
...
static void s3c3410x_interrupt_write(void  *state, uint32_t addr, uint32_t data)
{
	DEBUG("%s(addr:0x%x, data:0x%x)\n", __FUNCTION__, addr, data);
...
/* Timer Routine */
static void s3c3410x_timer_do_cycle(void  *state)
{
	int i, k;
	uint32_t tdat, count, empty_count;
	uint32_t cnt_up, cnt_divider, cnt_clock_divider = 1;
...
static void s3c3410x_timer_read(uint32_t offset, uint32_t *data, int index)
{
	switch (offset) {
		case 0: /* TDAT */
...
static void s3c3410x_timer_write(void  *state, uint32_t offset, uint32_t data, int index)
{
	DEBUG("%s(Timer%d, offset:0x%x, data:0x%x)\n", __FUNCTION__, index, offset, data);
...
/* UART Routine */
static void s3c3410x_uart_do_cycle(void  *state)
{
	int i;
	unsigned char buf;
	struct timeval tv;
...
static void s3c3410x_uart_read(uint32_t addr, uint32_t *data)
{
	switch (addr) {
		case ULCON:
			*data = io.ulcon;
...
static void s3c3410x_uart_write(void  *state, uint32_t addr, uint32_t data)
{
	int cnt = 0;
...
static void s3c3410x_dma_do_cycle(void  *state)
{
	int i;
...
static void s3c3410x_dma_read(uint32_t offset, uint32_t *data, int index)
{
	switch (offset) {
		case 0x0: /* DMASRC */
...
static void s3c3410x_dma_write(void  *state, uint32_t offset, uint32_t data, int index)
{
	DEBUG("%s(DMA%d, offset:0x%x, data:0x%x)\n", __FUNCTION__, index, offset, data);
...
/* IO Read Routine */
static uint32_t s3c3410x_io_read_word(void  *state, uint32_t addr)
{
	uint32_t data = -1;
...
static uint32_t s3c3410x_io_read_byte(void  *state, uint32_t addr)
{
	return s3c3410x_io_read_word(state, addr);
}


static uint32_t s3c3410x_io_read_halfword(void  *state, uint32_t addr)
{
	return s3c3410x_io_read_word(state, addr);
}
...
/* IO Write Routine */
static void s3c3410x_io_write_word(void  *state, uint32_t addr, uint32_t data)
{
	/* Interrupt */
	if (addr >= INTMOD && addr <= INTPRI7) {
...
static void s3c3410x_io_write_byte(void  *state, uint32_t addr, uint32_t data)
{
	s3c3410x_io_write_word(state, addr, data);
}


static void s3c3410x_io_write_halfword(void  *state, uint32_t addr, uint32_t data)
{
	s3c3410x_io_write_word(state, addr, data);
}


static void s3c3410x_io_do_cycle(void  *state)
{
...
void s3c3410x_mach_init(void  *state, machine_config_t *this_mach)
{
#if 0
	...
	state->Reg[1] = 377;
#endif
	((generic_arch_t *)state)->set_regval_by_id(1, 377);
