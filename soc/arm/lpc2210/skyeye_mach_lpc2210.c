...
typedef struct vic{
	...
	uint32_t VectAddr[15];
	uint32_t VectCntl[15];
} lpc2210_vic_t;
...
typedef struct lpc2210_io {
	...
	lpc2210_vic_t		vic;
	uint32_t			pinsel0;	/*Pin Select Register*/
	uint32_t			pinsel1;
	uint32_t			pinsel2;
	uint32_t			bcfg[4];	/*BCFG Extend Mem control*/
	uint32_t			vpbdiv;		/*VPB Divider control*/
        int             	tc_prescale;
	lpc2210_uart_t	uart[2];                 /* Receive data register */
	uint32_t		mmcr;			/*Memory mapping control register*/
	uint32_t		vibdiv;
...
void lpc2210_io_write_word(void *state, uint32_t addr, uint32_t data);
...
static void lpc2210_update_int(void *state)
{
	...
	interrupt_signal.arm_signal.firq = io.vic.FIQStatus ? Low_level : High_level;
	interrupt_signal.arm_signal.irq = io.vic.IRQStatus ? Low_level : High_level;
...
uint32_t	lpc2210_unfix_int(uint32_t val)
{
/*	uint32_t ret = 0;
	if (val & URXINT)
		ret |= (1 << 2);
...
uint32_t
lpc2210_uart_read(void *state, uint32_t addr,int i)
{
	uint32_t data;
...
uint32_t lpc2210_io_read_byte(void *state, uint32_t addr)
{
...
uint32_t lpc2210_io_read_halfword(void *state, uint32_t addr)
{
		return lpc2210_io_read_word(state,addr);
		//SKYEYE_OUTREGS(stderr);
		//exit(-1);
...
void lpc2210_io_write_halfword(void *state, uint32_t addr, uint32_t data)
{
	lpc2210_io_write_word(state,addr,data);
	//SKYEYE_OUTREGS(stderr);
	//exit(-1);
...
void lpc2210_mach_init(void *state, machine_config_t *this_mach)
{
#if 0
