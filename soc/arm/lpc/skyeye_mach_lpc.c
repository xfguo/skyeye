...
uint32_t lpc_io_read_word (void  * state, uint32_t addr);
void lpc_io_write_word (void  * state, uint32_t addr, uint32_t data);
//AJ2D--------------------------------------------------------------------------
...
typedef struct lpc_io
{
	...
	lpc_pll_t pll;
	uint32_t apbdiv;		/* APB divider register */
	lpc_timer_t timer[2];
	lpc_vic_t vic;
	uint32_t pinsel0;
	lpc_uart_t uart[2];	/* Receive data register */
	uint32_t memmap;		/*Memory mapping control register */
...
//        state->NfiqSig = (~(io.vic.risr&io.vic.ier& io.vic.)) ? LOW : HIGH;
	...
	interrupt_signal.arm_signal.firq = io.vic.fsr ? Low_level : High_level;
	interrupt_signal.arm_signal.irq = io.vic.isr  ? Low_level : High_level;
...
uint32_t
lpc_uart_read (void  * state, uint32_t addr, int i)
{
	uint32_t data;
...
void
lpc_uart_write (void  * state, uint32_t addr, uint32_t data, int i)
{
...
uint32_t
lpc_io_read_word (void  * state, uint32_t addr)
{
	/*
...
uint32_t
lpc_io_read_byte (void  * state, uint32_t addr)
{
...
uint32_t
lpc_io_read_halfword (void  * state, uint32_t addr)
{
	return lpc_io_read_word (state, addr);
	//SKYEYE_OUTREGS(stderr);
	//exit(-1);
...
void
lpc_io_write_word (void  * state, uint32_t addr, uint32_t data)
{
	/*
	 * The lpc system registers
	 */
...
void
lpc_io_write_byte (void  *state, uint32_t addr, uint32_t data)
{
...
void
lpc_io_write_halfword (void  *state, uint32_t addr, uint32_t data)
{
	lpc_io_write_word (state, addr, data);
	//SKYEYE_OUTREGS(stderr);
	//exit(-1);
...
void
lpc_mach_init (void  *state, machine_config_t * this_mach)
{
	...
	//state->mach_io.instr = (uint32_t *) & io.vic.isr;
