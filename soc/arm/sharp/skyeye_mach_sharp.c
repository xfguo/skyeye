...
static void
shp_io_write_byte (void *state, uint32_t addr, uint32_t data)
{
...
static void
shp_io_write_halfword (void *state, uint32_t addr, uint32_t data)
{
...
static void
shp_io_write_word (void *state, uint32_t addr, uint32_t data)
{
	shp_ioregnum_t ioregaddr = addr;
...
static uint32_t
shp_io_read_byte (void *state, uint32_t addr)
{
	u32 data = 0;
	shp_ioregnum_t ioregaddr = addr;
	switch (ioregaddr) {
...
static uint32_t
shp_io_read_halfword (void *state, uint32_t addr)
{
...
static uint32_t
shp_io_read_word (void *state, uint32_t addr)
{
	u32 data;
	shp_ioregnum_t ioregaddr = addr;
...
refresh_irq (void *state)
{
	...
	interrupt_signal.arm_signal.irq = shp_io.intc.status ? Low_level : High_level;
	send_signal(&interrupt_signal);
}

void
shp_mach_init (void *state, machine_config_t *mc)
{
#if 0
	...
	((generic_arch_t *)state)->set_regval_by_id(1, 89); /*BUGBUG: lubbock machine id. */
	shp_io_reset ();
