...
typedef struct cs89712_io
{
	...
	uint32_t intsr;		/* Interrupt status reg */
	uint32_t intmr;		/* Interrupt mask reg */

	uint32_t tcd[2];		/* Timer/counter data */
	uint32_t tcd_reload[2];	/* Last value written */
	int tc_prescale;

	uint32_t uartdr;		/* Receive data register */

	uint32_t lcdcon;		/* LCD control */
	uint32_t lcd_limit;	/* 0xc0000000 <= LCD buffer < lcd_limit */
} cs89712_io_t;
...
uint32_t
cs89712_io_read_halfword (void *state, uint32_t addr)
{
...
uint32_t
cs89712_io_read_word (void *state, uint32_t addr)
{
	uint32_t data = 0;
...
void
cs89712_io_write_halfword (void *state, uint32_t addr, uint32_t data)
{
...
void
cs89712_io_write_word (void *state, uint32_t addr, uint32_t data)
{
	uint32_t tmp;
...
#endif
	((generic_arch_t *)arch_instance)->set_regval_by_id(1, 107);  /* for cdb89712 arch id */
