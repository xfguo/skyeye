...
void
pxa250_io_write_byte (void  *state, uint32_t addr, uint32_t data)
{
	return;
}

void
pxa250_io_write_halfword (void  *state, uint32_t addr, uint32_t data)
{
	return;
}
...
pxa_update_intr (void *mach)
{
	...
	interrupt_signal.arm_signal.firq = pxa250_io.icfp ? Low_level : High_level;
	interrupt_signal.arm_signal.irq = pxa250_io.icip ? Low_level : High_level;
...
static void
pxa250_io_write_word (void  *state, uint32_t addr, uint32_t data)
{
...
#endif
	((generic_arch_t *)arch_instance)->set_regval_by_id(1, 89); /*lubbock machine id. */
	pxa250_io_reset ();
#if 0
