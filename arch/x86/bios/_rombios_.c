...
print_bios_banner()
{
  ...
  bios_printf(2, "apmbios " "pcibios " "eltorito " "rombios32 " "\n\n");
}
...
init_boot_vectors()
{
  ...
  write_word(0x9ff0, 0x0084, 0xFFFF);
    ...
    if (scan_code == 0x86)
    {
      ...
      while (!valid_choice) {
        ...
        else if (scan_code <= count)
        {
          valid_choice = 1;
          scan_code -= 1;
          write_word(0x9ff0, 0x0084, scan_code);
        }
      }
      bios_printf(2, "\n");
    }
...
real_mode:
 ...
 #endasm
      set_enable_a20(1);
      write_word(ES, regs.u.r16.si+0x38+0, 0xffff);
      write_word(ES, regs.u.r16.si+0x38+2, 0x0000);
      write_byte(ES, regs.u.r16.si+0x38+4, 0x000f);
      write_byte(ES, regs.u.r16.si+0x38+5, 0x9b);
      write_word(ES, regs.u.r16.si+0x38+6, 0x0000);
      BX = regs.u.r16.bx;
 #asm
      mov bx, sp
      SEG SS
...
Bit16u seq_nr;
{
  ...
  bootdev >>= 4 * seq_nr;
  bootdev &= 0xf;
  bootfirst = read_word(0x9ff0, 0x0084);
  if (bootfirst != 0xFFFF) {
    bootdev = bootfirst;
    write_word(0x9ff0, 0x0084, 0xFFFF);
    write_word(0x9ff0, 0x0082, 0xFFFF);
