 ...
 *       Filename:  i_save.c
 *
 *    Description:  Implementation of the SAVE instruction
 *
 *        Version:  1.0
...
31-30  29-25   24-19     18-14    13        12-5          4-0
+----+-------+--------+------------------------------------------+
| 10 | 00000 | 111100 |   rs1   | 1/0 |      simm13/rs2          |
+----+-------+--------+------------------------------------------+
...
#define SAVE_CYCLES    1
#define SAVE_CODE_MASK 0x81e00000
#define PRIVILEDGE  0
...
static int execute(void *state)
{
    ...
    {
        sum = REG(rs1) + REG(rs2);
        print_inst_RS_RS_RD("save", rs1, rs2, rd);
    }
    else
    {
        sum = REG(rs1) + sign_ext13(imm);
        print_inst_RS_IM13_RD("save", rs1, sign_ext13(imm), rd);
    }

    /*  Value of the WIM register   */
//    wim = WIMREG & 0x000000FF;
    wim = WIMREG;
    cwp = CWP;
