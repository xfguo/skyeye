...
#define OR_CYCLES    1
#define OR_CODE_MASK 0x80100000
#define PRIVILEDGE  0
...
static int execute(void *state)
{
    ...
    if( imm < 0 )
    {
        ...
        print_inst_RS_RS_RD("or", rs1, rs2, rd);
    }
    else
    {
        REG(rd) = REG(rs1) | sign_ext13(imm);
//        DBG("or reg[%d], 0x%x, reg[%d]\n", rs1, sign_ext13(imm), rd);
        print_inst_RS_IM13_RD("or", rs1, sign_ext13(imm), rd);
    }
...
static int disassemble(uint32 instr, void *state)
{
    ...
    if( (instr & OR_CODE_MASK) && (op == OP) && (op3 == OP3) )
    {
