 ...
 *       Filename:  i_srl.c
 *
 *    Description:  Implementation of the logical left shift.
 *
 *        Version:  1.0
...
#define SLL_CYCLES    1
#define SLL_CODE_MASK   0x81280000 
#define PRIVILEDGE  0
...
static int execute(void *state)
{
    ...
    REG(rd) = REG(rs1) << value;

//    DBG("sll reg[%d], 0x%x, reg[%d]\n", rs1, value, rd);
    print_inst_RS_IM13_RD("sll", rs1, value, rd);
//    DBG("sll %d, 0x%x, %d\n", REG(rs1), value, REG(rd));
...
static int disassemble(uint32 instr, void *state)
{
    ...
    if( (instr & SLL_CODE_MASK) && (op == OP) && (op3 == OP3) )
    {
