...
#define AND_CYCLES    1
#define AND_CODE_MASK 0x80080000
#define PRIVILEDGE  0
...
static int execute(void *state)
{
    ...
    if( imm < 0 )
    {
        ...
        print_inst_RS_RS_RD("and", rs1, rs2, rd);
//        DBG("and reg[%d], reg[%d], reg[%d]\n", rs1, rs2, rd);
    }
    else
    {
        REG(rd) = REG(rs1) & sign_ext13(imm);
        print_inst_RS_IM13_RD("and", rs1, sign_ext13(imm), rd);
//        DBG("and reg[%d], 0x%x, reg[%d]\n", rs1, sign_ext13(imm), rd);
    }
...
static int disassemble(uint32 instr, void *state)
{
    ...
    if( (instr & AND_CODE_MASK) && (op == OP) && (op3 == OP3) )
    {
