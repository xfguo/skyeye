...
+----+-------+--------+------------------------------------------+
| 11 | 00000 | 000010 |   rs1   | 1/0 |      simm12/rs2          |
+----+-------+--------+------------------------------------------+
...
#define LDUH_CYCLES    1
#define LDUH_CODE_MASK 0xc0100000
#define PRIVILEDGE  0
...
//        DBG("lduh [ reg[%d] + reg[%d] ], reg[%d]\n", rs1, rs2, rd);
        print_inst_LS_RS_RS_RD("lduh", rs1, rs2, rd);
    }
    else
    {
        addr = REG(rs1) + sign_ext13(imm);
//        DBG("lduh [ reg[%d] + %d ], reg[%d]\n", rs1, sign_ext13(imm), rd);
        print_inst_LS_RS_IM13_RD("lduh", rs1, sign_ext13(imm), rd);
    }
...
static int disassemble(uint32 instr, void *state)
{
    ...
    if( (instr & LDUH_CODE_MASK) && (op == OP) && (op3 == OP3) )
    {
