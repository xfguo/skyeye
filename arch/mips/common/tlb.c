...
void reset_tlb_mt(MIPS_State* mstate);
void set_tlb_entry_mt(MIPS_State* mstate, int n);
TLBEntry * probe_tlb_mt(MIPS_State* mstate, VA va);
int translate_vaddr_mt(MIPS_State* mstate, VA va, int type, PA * pa);
...
void
reset_tlb_mt(MIPS_State* mstate)
{
...
void
set_tlb_entry_mt(MIPS_State* mstate, int n)
{
    	int hash = tlb_hash(mstate->cp0[EntryHi]);
	MIPS_CPU_State* cpu = get_current_cpu();
	mips_core_t* cur_mstate = mstate;		/* save the cur tc mstate as current mstate */
...
TLBEntry *
probe_tlb_mt(MIPS_State* mstate, VA va)
{
    	int hash = tlb_hash(va);
    	TLBEntry* e; 
	MIPS_CPU_State* cpu = get_current_cpu();
...
translate_vaddr(MIPS_State* mstate, VA va, int type, PA * pa)
{
	...
	if(mstate->mt_flag)
		translate_vaddr_mt(mstate, va, type, pa);
	else{
...
int
translate_vaddr_mt(MIPS_State* mstate, VA va, int type, PA * pa)
{
	...
	//fprintf(stderr, "Warning:Can not find entry for mips tlb in %s,va=0x%x,pc=0x%x\n",__FUNCTION__, va, mstate->pc);
    	...
    	// Now, we are ready to probe the TLB.
	TLBEntry* entry = probe_tlb(cur_mstate, va);
    	if (!entry){
		process_tlb_refill(cur_mstate, type, va); // TLB miss
		return TLB_EXP; /* */
	}
	...
	// Filter TLB entries marked invalid.
    	if (!(lo & 0x2)){ /*test Valid bit in lo of TLB */
		//printf("In %s, invalid entry->lo=0x%x,va=0x%x,pc=0x%x\n", __FUNCTION__, lo, va, mstate->pc);
		process_tlb_invalid(cur_mstate, type, va); // TLB invalid.
		return TLB_EXP;
	}
    	// Fiter TLB entries marked read-only.
    	if (type == data_store && !(lo & 0x4)){
		//printf("In %s, tlb_modified exp!,pc=0x%x\n", __FUNCTION__, mstate->pc);
		process_tlb_modified(cur_mstate, va); // TLB Modified exception.
		return TLB_EXP;
	}
    	// Finally, retrieve the mapping.
    	//*pa = ((mstate->cp0[EntryLo0] & (bitsmask(11, 0))) | (va & (bitsmask(31, 12)))); //Shi yang 2006-08-11
