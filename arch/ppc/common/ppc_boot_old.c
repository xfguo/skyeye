...
void ppc_boot(){
	int i;
	load_initrd();
	if(!strcmp(skyeye_config.mach->machine_name, "mpc8572"))
		setup_boot_param(); /* MPC8572 */
	else if(!strcmp(skyeye_config.mach->machine_name, "mpc8560"))
		set_bootcmd(); /* MPC8560 */
	else{
		printf("Can not find suitable boot function for %s.", skyeye_config.mach->machine_name);
		return;
	}
...
#if 0
	if(!strcmp(skyeye_config.mach->machine_name, "mpc8572"))
		gCPU.ccsr.ccsr = 0xFFE00; /* Only for boot MPC8572 linux */ 
        else if(!strcmp(skyeye_config.mach->machine_name, "mpc8560"))
