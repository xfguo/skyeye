/*
    arm7ejs_mmu.c - ARM7EJS Memory Management Unit emulation.
...
#define ARM7EJS_MAIN_TLB() (&state->mmu.u.arm7ejs_mmu.main_tlb)
#define ARM7EJS_I_CACHE() (&state->mmu.u.arm7ejs_mmu.i_cache)

#define ARM7EJS_LOCKDOWN_TLB() (&state->mmu.u.arm7ejs_mmu.lockdown_tlb)
#define ARM7EJS_D_CACHE() (&state->mmu.u.arm7ejs_mmu.d_cache)
#define ARM7EJS_WB() (&state->mmu.u.arm7ejs_mmu.wb_t)

typedef struct arm7ejs_mmu_desc_s
{
	int main_tlb;
	int lockdown_tlb;
...
static arm7ejs_mmu_desc_t arm7ejs_mmu_desc = {
...
static fault_t arm7ejs_mmu_write (ARMul_State * state, ARMword va,
				    ARMword data, ARMword datatype);
static fault_t arm7ejs_mmu_read (ARMul_State * state, ARMword va,
				   ARMword * data, ARMword datatype);
...
mmu_wb_write_bytes (ARMul_State * state, wb_s * wb_t, ARMword pa,
                    ARMbyte * data, int n);
int
arm7ejs_mmu_init (ARMul_State * state)
{
	arm7ejs_mmu_desc_t *desc;
	cache_desc_t *c_desc;
	...
	desc = &arm7ejs_mmu_desc;
	if (mmu_tlb_init (ARM7EJS_MAIN_TLB (), desc->main_tlb)) {
		err_msg ("main_tlb init %d\n", -1);
		goto main_tlb_init_error;
	}

	c_desc = &desc->i_cache;
	if (mmu_cache_init (ARM7EJS_I_CACHE (), c_desc->width, c_desc->way,
			    c_desc->set, c_desc->w_mode)) {
		err_msg ("i_cache init %d\n", -1);
		goto i_cache_init_error;
	}

	if (mmu_tlb_init (ARM7EJS_LOCKDOWN_TLB (), desc->lockdown_tlb)) {
		err_msg ("lockdown_tlb init %d\n", -1);
		goto lockdown_tlb_init_error;
	}

	c_desc = &desc->d_cache;
	if (mmu_cache_init (ARM7EJS_D_CACHE (), c_desc->width, c_desc->way,
			    c_desc->set, c_desc->w_mode)) {
		err_msg ("d_cache init %d\n", -1);
		goto d_cache_init_error;
	}

	if (mmu_wb_init (ARM7EJS_WB (), desc->wb.num, desc->wb.nb)) {
		err_msg ("wb init %d\n", -1);
		goto wb_init_error;
	}
	return 0;
      ...
      wb_init_error:
	mmu_cache_exit (ARM7EJS_D_CACHE ());
      d_cache_init_error:
	mmu_tlb_exit (ARM7EJS_LOCKDOWN_TLB ());
      lockdown_tlb_init_error:
	mmu_cache_exit (ARM7EJS_I_CACHE ());
      i_cache_init_error:
	mmu_tlb_exit (ARM7EJS_MAIN_TLB ());
      main_tlb_init_error:
	return -1;
}

void
arm7ejs_mmu_exit (ARMul_State * state)
{
	mmu_wb_exit (ARM7EJS_WB ());
	mmu_cache_exit (ARM7EJS_D_CACHE ());
	mmu_tlb_exit (ARM7EJS_LOCKDOWN_TLB ());
	mmu_cache_exit (ARM7EJS_I_CACHE ());
	mmu_tlb_exit (ARM7EJS_MAIN_TLB ());
...
static fault_t
arm7ejs_mmu_load_instr (ARMul_State * state, ARMword va, ARMword * instr)
{
	fault_t fault;
	tlb_entry_t *tlb;
	cache_line_t *cache;
	...
	if (MMU_Enabled) {
		...
		/*translate tlb */
		fault = translate (state, va, ARM7EJS_MAIN_TLB (), &tlb);
		if (fault) {
			d_msg ("translate\n");
			return fault;
		}
	...
	cache = mmu_cache_search (state, ARM7EJS_I_CACHE (), va);
	if (cache) {
		*instr = cache->
			data[va_cache_index (va, ARM7EJS_I_CACHE ())];
		return 0;
	}
	...
	if (c) {
		...
		debug_count++;
		cache = mmu_cache_alloc (state, ARM7EJS_I_CACHE (), va, pa);
		index = va_cache_index (va, ARM7EJS_I_CACHE ());
		*instr = cache->
			data[va_cache_index (va, ARM7EJS_I_CACHE ())];
	}
	else
...
static fault_t
arm7ejs_mmu_read_byte (ARMul_State * state, ARMword virt_addr,
			 ARMword * data)
{
	//ARMword temp,offset;
	fault_t fault;
	fault = arm7ejs_mmu_read (state, virt_addr, data, ARM_BYTE_TYPE);
	return fault;
}

static fault_t
arm7ejs_mmu_read_halfword (ARMul_State * state, ARMword virt_addr,
			     ARMword * data)
{
	//ARMword temp,offset;
	fault_t fault;
	fault = arm7ejs_mmu_read (state, virt_addr, data,
				    ARM_HALFWORD_TYPE);
	return fault;
}

static fault_t
arm7ejs_mmu_read_word (ARMul_State * state, ARMword virt_addr,
			 ARMword * data)
{
	return arm7ejs_mmu_read (state, virt_addr, data, ARM_WORD_TYPE);
}
...
static fault_t
arm7ejs_mmu_read (ARMul_State * state, ARMword va, ARMword * data,
		    ARMword datatype)
{
	fault_t fault;
	tlb_entry_t *tlb;
	...
	if (MMU_Disabled) {
		...
		if (datatype == ARM_BYTE_TYPE)
			...
			printf ("SKYEYE:1 arm7ejs_mmu_read error: unknown data type %d\n", datatype);
			skyeye_exit (-1);
		}
	...
	if (fault)
		return fault;
	/*search main cache */
	cache = mmu_cache_search (state, ARM7EJS_D_CACHE (), va);
	if (cache) {
		*data = cache->
			data[va_cache_index (va, ARM7EJS_D_CACHE ())];
		goto datatrans;
		//return 0;
	}
	...
	if ((pa >= 0xe0000000) && (pa < 0xe8000000)) {
		if (tlb_c_flag (tlb)) {
			//if (tlb_b_flag(tlb)){
			//      mmu_cache_soft_flush(state, ARM7EJS_D_CACHE(), pa);
			//}
			mmu_cache_soft_flush (state, ARM7EJS_D_CACHE (),
					      pa);
		}
		return 0;
	}
	...
	if (tlb_b_flag (tlb))
		mmu_wb_drain_all (state, ARM7EJS_WB ());
	...
	if (tlb_c_flag (tlb) && MMU_DCacheEnabled) {
		...
		*data = cache->data[va_cache_index (va, cache_t)];
		...
		if (datatype == ARM_BYTE_TYPE)
			...
			printf ("SKYEYE:2 arm7ejs_mmu_read error: unknown data type %d\n", datatype);
			skyeye_exit (-1);
		}
		return 0;
	}
...
static fault_t
arm7ejs_mmu_write_byte (ARMul_State * state, ARMword virt_addr,
			  ARMword data)
{
	return arm7ejs_mmu_write (state, virt_addr, data, ARM_BYTE_TYPE);
}

static fault_t
arm7ejs_mmu_write_halfword (ARMul_State * state, ARMword virt_addr,
			      ARMword data)
{
	return arm7ejs_mmu_write (state, virt_addr, data,
				    ARM_HALFWORD_TYPE);
}

static fault_t
arm7ejs_mmu_write_word (ARMul_State * state, ARMword virt_addr,
			  ARMword data)
{
	return arm7ejs_mmu_write (state, virt_addr, data, ARM_WORD_TYPE);
}



static fault_t
arm7ejs_mmu_write (ARMul_State * state, ARMword va, ARMword data,
		     ARMword datatype)
{
	tlb_entry_t *tlb;
	cache_line_t *cache;
	...
	if (MMU_Disabled) {
		...
		if (datatype == ARM_BYTE_TYPE)
			...
			printf ("SKYEYE:1 arm7ejs_mmu_write error: unknown data type %d\n", datatype);
			skyeye_exit (-1);
		}
...
ARMword
arm7ejs_mmu_mrc (ARMul_State * state, ARMword instr, ARMword * value)
{
	...
	case MMU_CACHE_OPS:
		/* TODO */
		//arm7ejs_mmu_cache_ops(state, instr, value);
		break;
...
/* ARM7EJS Cache Operation, P44
 * */
void
arm7ejs_mmu_cache_ops (ARMul_State * state, ARMword instr, ARMword value)
{
	int CRm, OPC_2;
	...
	if (OPC_2 == 0 && CRm == 7) {
		mmu_cache_invalidate_all (state, ARM7EJS_I_CACHE ());
		mmu_cache_invalidate_all (state, ARM7EJS_D_CACHE ());
		return;
	}
	 ...
	 **/
	if (OPC_2 == 1 && CRm == 5) {
		mmu_cache_invalidate (state, ARM7EJS_I_CACHE (), value);
		return;
	}
	/* Invalidate ICache single entry Set/way */
	if (OPC_2 == 2 && CRm == 5) {
		mmu_cache_invalidate_by_index (state, ARM7EJS_I_CACHE (),
					       value);
		return;
	}
	/* TODO:
	 ...
	 * */
	if (OPC_2 == 1 && CRm == 6) {
		mmu_cache_invalidate (state, ARM7EJS_D_CACHE (), value);
		return;
	}
	/* Invalidate DCache single entry Set/way */
	if (OPC_2 == 2 && CRm == 6) {
		mmu_cache_invalidate_by_index (state, ARM7EJS_D_CACHE (),
					       value);
		return;
	}
	 ...
	 * */
	if (OPC_2 == 1 && CRm == 0xa) {
		mmu_cache_clean (state, ARM7EJS_D_CACHE (), value);
		return;
	}
	 ...
	 * */
	if (OPC_2 == 2 && CRm == 0xa) {
		mmu_cache_clean_by_index (state, ARM7EJS_D_CACHE (), value);
		return;
	}
...
static void
arm7ejs_mmu_tlb_ops (ARMul_State * state, ARMword instr, ARMword value)
{
	int CRm, OPC_2;
	...
	if (OPC_2 == 0 && (CRm == 0x7) || (CRm == 0x6) || (CRm == 0x5)) {
		mmu_tlb_invalidate_all (state, ARM7EJS_MAIN_TLB ());
		//mmu_tlb_invalidate_all(state, ARM7EJS_LOCKDOWN_TLB());
		return;
	}

	if (OPC_2 == 1 && (CRm == 0x5 || (CRm == 0x7) || (CRm == 0x6))) {
		mmu_tlb_invalidate_entry (state, ARM7EJS_MAIN_TLB (),
					  value);
		return;
	}
...
static void
arm7ejs_mmu_cache_lockdown (ARMul_State * state, ARMword instr,
			      ARMword value)
{
	int OPC_2 = BITS (5, 7) & 0x7;
}
static void
arm7ejs_mmu_tlb_lockdown (ARMul_State * state, ARMword instr, ARMword value)
{
}


static ARMword
arm7ejs_mmu_mcr (ARMul_State * state, ARMword instr, ARMword value)
{
...
//              printf("mmu_mcr wrote DACR         ");
	...
	case MMU_CACHE_OPS:
		...
		break;
	case MMU_TLB_OPS:
		arm7ejs_mmu_tlb_ops (state, instr, value);
		break;
...
static int
arm7ejs_mmu_v2p_dbct (ARMul_State * state, ARMword virt_addr,
			ARMword * phys_addr)
{
	fault_t fault;
	tlb_entry_t *tlb;
	...
	if (MMU_Enabled) {
		...
		/*translate tlb */
		fault = translate (state, virt_addr, ARM7EJS_MAIN_TLB (),
				   &tlb);
		if (fault) {
			d_msg ("translate\n");
			return fault;
...
/*arm7ejs mmu_ops_t*/
mmu_ops_t arm7ejs_mmu_ops = {
	arm7ejs_mmu_init,
	arm7ejs_mmu_exit,
	arm7ejs_mmu_read_byte,
	arm7ejs_mmu_write_byte,
	arm7ejs_mmu_read_halfword,
	arm7ejs_mmu_write_halfword,
	arm7ejs_mmu_read_word,
	arm7ejs_mmu_write_word,
	arm7ejs_mmu_load_instr,
	arm7ejs_mmu_mcr,
	arm7ejs_mmu_mrc,
//teawater add for arm2x86 2005.06.19-------------------------------------------
	arm7ejs_mmu_v2p_dbct,
//AJ2D--------------------------------------------------------------------------
};
