...
typedef struct net_rtl8019_io
{
	...
	uint8 PROM[12];		// 12 bytes in Prom for MAC addr.

	uint8 *sram;
	uint32 remote_read_offset;
	uint32 remote_write_offset;

	uint32 remote_read_count;
	uint32 remote_write_count;
...
static uint8 rtl8019_output (struct device_desc *dev, uint8 * buf, uint16 packet_len);
