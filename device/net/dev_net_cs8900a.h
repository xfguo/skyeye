...
typedef struct net_cs8900a_io
{
	...
	uint16 dma_channel_num;
	uint16 dma_start_frame;
	uint16 dma_frame_count;

	uint16 rxdma_byte_count;

	uint16 mem_base[2];
	uint16 boot_prom_base[2];
	uint16 boot_prom_mask[2];


	uint16 eeprom_cmd;
	uint16 eeprom_data;
	uint16 eeprom[64];		// 64 (16 bits word) in Prom.
	int eeprom_writable;
	...
	/*
	   ...
	   uint16 sc_tx_cfg;
	   uint16 sc_tx_event;
	   uint16 sc_tx_cmd_status;
	   uint16 sc_buf_cfg;
	   uint16 sc_buf_event;
	   uint16 sc_rx_miss_count;
	   uint16 sc_tx_col_count;
	   uint16 sc_line_ctrl;
	   uint16 sc_line_status;
	   uint16 sc_self_ctrl;
	   uint16 sc_self_status;
	   uint16 sc_bus_ctrl;
	   uint16 sc_bus_status;
	   uint16 sc_test_ctrl;
	 */
...
static uint8 cs8900a_output (struct device_desc *dev, uint8 * buf, uint16 packet_len);
