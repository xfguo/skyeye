...
static void
write_cr (struct device_desc *dev, uint8 data)
{
	struct net_device *net_dev = (struct net_device *) dev->dev;
	struct net_rtl8019_io *io = (struct net_rtl8019_io *) dev->data;
	uint8 startpage;
	...
	if (data & CMD_XMIT) {
		...
		packet_len = (((uint16) io->TBCR1 << 8) | (io->TBCR0));
		packet_len &= 0xffff;
	...
	if ((data & CMD_READ) && (data & CMD_RUN)) {
		rtl8019_len = (((uint16) io->RBCR1 << 8) | io->RBCR0);
		io->remote_read_offset =
			(uint16) (io->RSAR1 << 8 | io->RSAR0) -
			(uint16) (START_PAGE * PAGE_SIZE);
	...
	if ((data & CMD_WRITE) && (data & CMD_RUN)
		...
		io->remote_write_count = (((uint16) io->RBCR1 << 8) | io->RBCR0);
...
static void
remote_write_word (struct device_desc *dev, uint16 data)
{
	struct net_device *net_dev = (struct net_device *) dev->dev;
	struct net_rtl8019_io *io = (struct net_rtl8019_io *) dev->data;
...
static void
remote_write_byte (struct device_desc *dev, uint8 data)
{
	struct net_device *net_dev = (struct net_device *) dev->dev;
	struct net_rtl8019_io *io = (struct net_rtl8019_io *) dev->data;
...
net_rtl8019_reset (struct device_desc *dev)
{
	...
	/*init PROM */
	io->PROM[0] = (uint8) net_dev->macaddr[0];	//MACADDR0;
	io->PROM[1] = (uint8) net_dev->macaddr[0];	//MACADDR0;
	io->PROM[2] = (uint8) net_dev->macaddr[1];	//MACADDR1;
	io->PROM[3] = (uint8) net_dev->macaddr[1];	//MACADDR1;
	io->PROM[4] = (uint8) net_dev->macaddr[2];	//MACADDR2;
	io->PROM[5] = (uint8) net_dev->macaddr[2];	//MACADDR2;
	io->PROM[6] = (uint8) net_dev->macaddr[3];	//MACADDR3;
	io->PROM[7] = (uint8) net_dev->macaddr[3];	//MACADDR3;
	io->PROM[8] = (uint8) net_dev->macaddr[4];	//MACADDR4;
	io->PROM[9] = (uint8) net_dev->macaddr[4];	//MACADDR4;
	io->PROM[10] = (uint8) net_dev->macaddr[5];	//MACADDR5;
	io->PROM[11] = (uint8) net_dev->macaddr[5];	//MACADDR5;
	...
	if (io->sram != NULL) {
		...
		io->sram = (uint8 *) malloc (PAGE_NUM * PAGE_SIZE);
	}
...
static int
net_rtl8019_read_halfword (struct device_desc *dev, uint32 addr, uint16 * data)
{
	struct net_device *net_dev = (struct net_device *) dev->dev;
	struct net_rtl8019_io *io = (struct net_rtl8019_io *) dev->data;

	int offset = (uint8) (addr - dev->base);
	int ret = ADDR_HIT;
...
static int
net_rtl8019_read_byte (struct device_desc *dev, uint32 addr, uint8 * data)
{
	struct net_device *net_dev = (struct net_device *) dev->dev;
	struct net_rtl8019_io *io = (struct net_rtl8019_io *) dev->data;
...
static int
net_rtl8019_write_halfword (struct device_desc *dev, uint32 addr, uint16 data)
{
	struct net_device *net_dev = (struct net_device *) dev->dev;
	struct net_rtl8019_io *io = (struct net_rtl8019_io *) dev->data;
	...
	if (offset == 0x10) {	//remote write
		if (io->DCR & 0x1) {
			remote_write_word (dev, (uint16) data);
		}
	}
	return ret;
...
static int
net_rtl8019_write_byte (struct device_desc *dev, uint32 addr, uint8 data)
{
	struct net_device *net_dev = (struct net_device *) dev->dev;
	struct net_rtl8019_io *io = (struct net_rtl8019_io *) dev->data;
	...
	if (offset == 0x10) {	//remote write
		/* FIXME: don't use DCR here. */
		//if (io->DCR & 0x2) {
		remote_write_byte (dev, (uint8) data);
		//}
		return ret;
	}
	if (offset == 0x1f) {	//reset
...
rtl8019_input (struct device_desc *dev)
{
	...
	uint8 buf[1600];
	uint8 frame_header[4];
	uint8 *bufptr;
	uint8 *sramptr;
	uint16 i, j, len;
	uint32 free_pages, occupy_pages, next_page;
...
static uint8
rtl8019_output (struct device_desc *dev, uint8 * buf, uint16 packet_len)
{
	struct net_device *net_dev = (struct net_device *) dev->dev;
	struct net_rtl8019_io *io = (struct net_rtl8019_io *) dev->data;
	int len;
