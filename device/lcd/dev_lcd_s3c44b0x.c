...
static uint32 s3c44b0x_lookup_color_MONO(struct lcd_device *lcd_dev, uint32 color)
{
	return (color ? 0xffffff : 0x000000);
}


static uint32 s3c44b0x_lookup_color_G4(struct lcd_device *lcd_dev, uint32 color)
{
	struct lcd_s3c44b0x_io *io = (struct lcd_s3c44b0x_io*)lcd_dev->device_priv;

	uint8 gray = (io->lcdbluelut >> ((color & 0x3) << 2)) & 0xf;

	return (((uint32)((gray << 4) | 0xf) << 16) | ((uint32)((gray << 4) | 0xf) << 8) | (uint32)((gray << 4) | 0xf));
}


static uint32 s3c44b0x_lookup_color_G16(struct lcd_device *lcd_dev, uint32 color)
{
	uint8 gray = color & 0xf;

	return (((uint32)((gray << 4) | 0xf) << 16) | ((uint32)((gray << 4) | 0xf) << 8) | (uint32)((gray << 4) | 0xf));
}


static uint32 s3c44b0x_lookup_color_C256(struct lcd_device *lcd_dev, uint32 color)
{
	struct lcd_s3c44b0x_io *io = (struct lcd_s3c44b0x_io*)lcd_dev->device_priv;
	uint8 c = color & 0xff;

	uint8 r = (io->lcdredlut >> ((c >> 5) << 2)) & 0xf;
	uint8 g = (io->lcdgreenlut >> (((c >> 2) & 0x7) << 2)) & 0xf;
	uint8 b = (io->lcdbluelut >> ((c & 0x3) << 2)) & 0xf;

	return (((uint32)((r << 4) | 0xf) << 16) | ((uint32)((g << 4) | 0xf) << 8) | (uint32)((b << 4) | 0xf));
}
...
int lcd_s3c44b0x_read_word(struct device_desc *dev, uint32 addr, uint32 *data)
{
	struct lcd_s3c44b0x_io *io = (struct lcd_s3c44b0x_io*)dev->data;
...
int lcd_s3c44b0x_write_word(struct device_desc *dev, uint32 addr, uint32 data)
{
	struct lcd_s3c44b0x_io *io = (struct lcd_s3c44b0x_io*)dev->data;
...
static int lcd_s3c44b0x_filter_read(struct device_desc *dev, uint32 addr, uint32 *data, size_t count)
{
	struct lcd_device *lcd_dev = (struct lcd_device*)dev->dev;
	struct lcd_s3c44b0x_io *io = (struct lcd_s3c44b0x_io*)dev->data;
...
static int lcd_s3c44b0x_filter_write(struct device_desc *dev, uint32 addr, uint32 data, size_t count)
{
	struct lcd_device *lcd_dev = (struct lcd_device*)dev->dev;
	struct lcd_s3c44b0x_io *io = (struct lcd_s3c44b0x_io*)dev->data;
