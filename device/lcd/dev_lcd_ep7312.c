...
ep7312_changed (struct device_desc *dev)
{
	...
	uint32 lcdcon = io->lcdcon;
	uint32 vbufsiz = lcdcon & VBUFSIZ;
	uint32 linelen = (lcdcon & LINELEN) >> LINELEN_SHIFT;
...
int
lcd_ep7312_read_word (struct device_desc *dev, uint32 addr, uint32 * data)
{
	struct lcd_device *lcd_dev = (struct lcd_device *) dev->dev;
	struct lcd_ep7312_io *io = (struct lcd_ep7312_io *) dev->data;
...
int
lcd_ep7312_write_word (struct device_desc *dev, uint32 addr, uint32 data)
{
	struct lcd_device *lcd_dev = (struct lcd_device *) dev->dev;
	struct lcd_ep7312_io *io = (struct lcd_ep7312_io *) dev->data;
