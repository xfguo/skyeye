...
static uint32 colors1b[2] =
{
	0x000000,0xffffff
};


static uint32 colors4b[16] =
{
...
static void SkyEyeLCD_DMAConvertCallback(uint32 c, uint32 **buf_addr, void *noused)
{
	*(*buf_addr) = c;
	*buf_addr = ++(*buf_addr);
}
...
static int win32_lcd_filter_write(struct lcd_device *lcd_dev, uint32 addr, uint32 data, size_t count)
{
