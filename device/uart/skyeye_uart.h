...
#define UART_SIM_TERM		3	/* use extern term as fd */
...
struct uart_option
{
	...
	/* 
	 * if we use stdio mode, we should take a term program as output 
	 * terminal. Or by default, we will mix the skyeye output with uart 
	 * output 
	 */
	char term_program[MAX_STR_NAME]; 	
	char converter[MAX_STR_NAME];
};
