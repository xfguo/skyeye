...
int
flash_intel_read_byte (struct device_desc *dev, uint32 addr, uint8 * data)
{
	...
	bigendSig = (arch_instance->endianess == Big_endian)?1:0;
	//offset = (((uint32) state->bigendSig * 3) ^ (addr & 3)) << 3;
	offset = (((uint32) bigendSig * 3) ^ (addr & 3)) << 3;
	*data = (temp >> offset & 0xffL);
	//printf("Not finished porting, please read the above code carefully.\n");
	//printf("In %s, mem_read addr=0x%x, data=0x%x, temp=0x%x\n", __FUNCTION__, addr, *data, temp);
	return ret;
}

int
flash_intel_read_halfword (struct device_desc *dev, uint32 addr, uint16 * data)
{
	struct machine_config *mc = (struct machine_config *) dev->mach;
	//ARMul_State *state = (ARMul_State *) mc->state;
	uint32 temp, offset;
	...
	flash_intel_read_word (dev, addr, &temp);
#if 0 /* Fixme */
	offset = (((uint32) state->bigendSig * 2) ^ (addr & 2)) << 3;
...
int
flash_intel_read_word (struct device_desc *dev, uint32 addr, uint32 * data)
{
	struct flash_device *flash_dev = (struct flash_device *) dev->dev;
	struct flash_intel_io *io = (struct flash_intel_io *) dev->data;
	struct machine_config *mc = (struct machine_config *) dev->mach;
...
int
flash_intel_write_byte (struct device_desc *dev, uint32 addr, uint8 data)
{
	int ret = ADDR_HIT;
...
int
flash_intel_write_halfword (struct device_desc *dev, uint32 addr, uint16 data)
{
	int ret = ADDR_HIT;
...
void
init_wsm (struct device_desc *dev, uint32 addr, uint32 data)
{
	struct flash_intel_io *io = (struct flash_intel_io *) dev->data;
...
int
flash_intel_write_word (struct device_desc *dev, uint32 addr, uint32 data)
{
	struct machine_config *mc = (struct machine_config *) dev->mach;
	struct flash_intel_io *io = (struct flash_intel_io *) dev->data;
	//ARMul_State *state = (ARMul_State *) mc->state;
