...
{
	uint8 *lock;		/* sector lock */
	uint32 size;		/* byte size */
	uint32 read_mode;		/* mode for read operation */
	uint32 wsm_mode;		/* write state machine */

	/* VPEN pin: should controlled by EGPIO ??? */
	uint8 vpen;

	/* latch address&data for each handlers */
	uint32 program_latch_addr;
	uint32 program_latch_data;
	uint32 progbuf_latch_addr;
	uint32 erase_latch_addr;
	...
	uint32 pb_count, pb_loaded;
	uint32 pb_start;		/* start address */
	uint32 pb_buf[INTEL_WRITEBUFFER_SIZE];	/* write buffer */
	 ...
	 */
	uint8 program_busy, progbuf_busy, erase_busy, lock_busy;
	uint8 program_suspended, progbuf_suspended, erase_suspended;
	uint8 protection_error, program_setlb_error, erase_clearlb_error,
		program_volt_error;
} flash_intel_io_t;
