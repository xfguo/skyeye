...
static inline FUNCTION_CONST uint32 bswap_word(uint32 data)
{
	return (data>>24)|((data>>8)&0xff00)|((data<<8)&0xff0000)|(data<<24);
}

static inline FUNCTION_CONST uint64 bswap_dword(uint64 data)
{
	return (((uint64)ppc_bswap_word(data)) << 32) | (uint64)ppc_bswap_word(data >> 32);
}

static inline FUNCTION_CONST uint16 bswap_half(uint16 data)
{
