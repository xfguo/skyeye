	...
	BasicBlock *bb)
{
	...
	if (size == 80 && (cpu->dyncom_engine->flags & CPU_FLAG_FP80) == 0) {
		arch_synthesize_fp80_store(cpu, index * 2, v, bb);
	} else if (size == 128 && (cpu->dyncom_engine->flags & CPU_FLAG_FP128) == 0) {
...
{
#if 0
	if (cpu->dyncom_engine->flags & CPU_FLAG_FP80) {
		Type const *type = v->getType();
		if (type->isInteger()) {
			if (type->getPrimitiveSizeInBits() < 32)
...
{
#if 0
	if (cpu->dyncom_engine->flags & CPU_FLAG_FP128) {
		Type const *type = v->getType();
		if (type->isInteger()) {
			if (type->getPrimitiveSizeInBits() < 32)
